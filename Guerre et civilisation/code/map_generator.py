...
def new_map_file(nom_fichier, taillex, tailley, full_of="vide"):
    ...
    temp = Maps(taillex, tailley, full_of)  # creation de map équivalentes
    text = ""
...
def change_type(doc, screen):  # TODO : mettre cela en place pour que cela fonctionne avec les fenetres
    work_map = Maps()
    ...
    while True:
        if pygame.mouse.get_pressed()[0] == 1:  # clique droit de la souris
            x, y = cases(pygame.mouse.get_pos(), data)  # récupération de la position de la souris
            x1, y1 = x // data[1], y // data[2]  # mise en relation de la position et des cases
            type_terrain = liste_terrain.index(
                work_map.contenu[x1][y1].state)  # on recupere la position du nom du terrain dans la liste
