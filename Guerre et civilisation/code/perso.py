...
class Perso:  # permet de creer des objets personnages(en mouvement sur la map)
    def __init__(self, ref, stat=None):  # un identifiant, et une liste de stats de la forme [[a],[x]]
        if stat is None:  # (si non donné en argument : realisé mais vide)
            stat = [[], []]
        self.ref = ref
        self.stat = stat
        self.graphism = None
    ...
    def set_coord(self, x, y):  # mise en place et modifications des coordonnees du personnage sur les maps
        self.coord[0] = x
        self.coord[1] = y

    def dif_travel(self, typ, nb):
        # calcul de la difficulté d'un voyage (type du terrain, deplacement sans la difficulté)
    ...
    def add_spe(self, special):  # permet d'ajouter une capacité speciale
        self.spe.append(special)

    def del_spe(self, special):  # permet de retirer une capacité speciale
    ...
    def change_stat(self, name_stat, nb):
        # changement de valeur d'une stat deja possedé (nom de la stat, nouvelle valeur)
    ...
    def get_stat(self, name_stat):  # retourne l'etat de la stat "name_stat"
    ...
    def print_perso(self, map_use, screen):
        ...
        position = self.coord  # recuperation de la position des perso
        ...
        if 0 <= position[0] < len(map_use.contenu) and 0 <= position[1] < len(map_use.contenu[0]):
            # securité pour eviter les dépassement hors tableau
            ...
            # remplacement de l'objet dans le tableau (on peut y faire en meme temps pour d'autres Perso)
        motor.condense_screen()
        pygame.display.flip()

    def test_move(self, maps, liste_perso=None):
        # affiche la map avec la valeur nescessaire au deplacement (avec la liste des position deja prise)
        ...
        save = copy.deepcopy(maps.contenu)  # on cree une sauvegarde independante de la carte de base
        liste = []  # creation de la liste dans laquelle on ira stocker les differentes valeurs de déplacement
        ...
        nb = 0
        # mise a 0 du curseur qui indiquera l'avancement dans ce travail (et le numero des cases que l'on va modifier)
        for j in range(maxi):
            # on refait l'operation jusqu'a ce que cela soit impossible d'avoir une case avec un nombre plus grand
        ...
        for i in range(len(save)):  # pour chaque case du terrain
            for j in range(len(save[0])):
                if work[i][j].get_graphism() < 0:  # si la valeur est negative, alors ce sont des terrains inaccessible
...
def print_perso(map_use, screen, liste_perso):
    ...
    for i in liste_perso:
        ...
        if 0 <= position[0] < len(map_use.contenu) and 0 <= position[1] < len(map_use.contenu[0]):
            # pour eviter les dépassement hors tableau
